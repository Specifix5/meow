...
import thisWillLoadEverythingFr from "./utils/loadEverythingForMePleaseOnegaishimasu";
import { ShoukoClient } from "./utils/shouko/client";
import { SHOUKOVERSION } from "./utils/constants";

dotenv.config({ path: ".env" });

(DefaultWebSocketManagerOptions.identifyProperties as { browser: string }).browser = "Discord Android";
...
client.logger.info("Loading Maidbot " + SHOUKOVERSION + ", attempting to connect to discord gateway");
thisWillLoadEverythingFr(client);

export const restClient = new REST({ version: "10" }).setToken(process.env.CLIENT_TOKEN!);

client.login(process.env.CLIENT_TOKEN).catch((err) => client.logger.error("Unable to log in: " + err));
